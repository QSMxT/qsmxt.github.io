...
This repository serves three things at `https://qsmxt.github.io/`:
...
3. **`recon/`** — the QSMxT Recon page. Unlike the other four tools, QSMxT Recon has no
   repo of its own to host a site from: the package is built from the
   [neurodesk/openrecon](https://github.com/neurodesk/openrecon/tree/main/recipes/qsmxt)
   recipe and cannot be distributed publicly, so the hub carries the instructions.
...
| [QSMxT Recon](https://qsmxt.github.io/recon/) | scanner — the pipeline as an OpenRecon app | `recon/` (this repo) |
...
site's own header. It is dependency-free and stack-agnostic (the sites use
Astro/Starlight, vanilla HTML, Alpine/Tailwind, and rustdoc). Each site
...
- `data-current` — highlights the current project (`rs`, `recon`, `xt`, `bly`, `ci`, `hub`).
...
- On `localhost` / `127.0.0.1` the script and all project links resolve to sibling paths
  (`/QSM.rs/`, `/recon/`, `/QSMxT/`, `/qsmbly/`, `/QSM-CI/`) so the whole ecosystem can be
  previewed from one local static server.
...
- **Colours.** Each tool has an accent that signals its role: QSM.rs orange (engine),
  QSMxT Recon rose (scanner), QSMxT purple (pipeline), QSMbly blue (browser), QSM-CI green
  (benchmark). The bar and hub use all five; each individual site leans on its own accent.
